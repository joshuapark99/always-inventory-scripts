...
def getInfo(sku,path,row,col):
    ...
    for col2 in range(col,style_sheet.max_column+1):
...
if __name__ == "__main__":
    ...
    for counter in range(2,read_sheet.max_row+1):
        ...
        object = getInfo(sku, path, row, col)
        write_sheet.cell(counter,1).value = read_sheet.cell(counter,1).value
        write_sheet.cell(counter,2).value = read_sheet.cell(counter,2).value
        write_sheet.cell(counter,3).value = read_sheet.cell(counter,3).value
